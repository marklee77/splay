...
class SplaydProtocol
    ...
    ## scenario in cluster deployments.
      ...
      # TODO ensure not violating anything in the protocol
      # Switch if anything breaks
      # Probably a new message is better with a proper message for this as well
      if true then
        ...
        in_table.each do |lib_pair|
         $log.info(lib_pair['name'])
         $log.info(lib_pair['sha1'])
         db_lib = $db.select_one("SELECT * FROM libs WHERE lib_sha1='#{lib_pair['sha1']}'")

         if not db_lib then
           db_lib = $db.select_one("SELECT * FROM libs WHERE lib_name='#{lib_pair['name']}' AND 
                                      lib_os='#{@splayd.row['os']}' AND lib_arch='#{@splayd.row['architecture']}' ")
           lib_code = ""
           if db_lib then
             lib_code = db_lib['lib_blob']
           end
           
           tmp_table = Hash.new
           tmp_table['name'] = lib_pair['name']
           tmp_table['code'] = lib_code
           out_table.push(tmp_table)
         end
        end
        $log.info("out_table = #{out_table}")
        #INSERT NEW ONES
        
        out_table = JSON.unparse(out_table)
        @so.write out_table
      end
