...
# source code as a string
  ...
  def pre_submit_job(name, lib_filename, lib_os, lib_arch, lib_sha1)
    ret = Hash.new
    
    #existing_lib = 
    #$db.select_all("SELECT * FROM libs WHERE lib_name='#{lib_filename}'") do |a_lib|
    #  if a_lib['lib_os'] == lib_os && a_lib['lib_arch'] == lib_arch && a_lib['lib_sha1'] == lib_sha1 then
    #    ret['ok'] = true 
    #    ret['message'] = "lib in db"
    #  end
    #end
    #existing_lib = $db.select_one("SELECT * FROM libs WHERE lib_sha1='#{lib_sha1}'")
    #admin = $db.select_one("SELECT * FROM users WHERE login='#{admin_username}'")
    #set = $db.select_all("SELECT * FROM libs WHERE lib_sha1='#{lib_sha1}'")
    #count = set.length
    existing_lib = $db.select_one("SELECT * FROM libs WHERE lib_sha1='#{lib_sha1}'")
    if existing_lib then
      ...
      ret['ok'] = false
      ret['message'] = "lib not in db"
    end
    
    return ret
  end
  
  def submit_lib(lib_name, lib_os, lib_arch, lib_sha1, lib_code)
    ...
    if existing_lib then
      $db.do("UPDATE libs SET lib_sha1='#{lib_sha1}', lib_blob='#{addslashes(lib_code)}' WHERE id='#{existing_lib['id']}'")
      ret['ok'] = true
      ret['message'] = "existing_lib updated"
    else
      $db.do("INSERT INTO libs SET lib_name='#{lib_name}', lib_os='#{lib_os}', lib_arch='#{lib_arch}', lib_sha1='#{lib_sha1}', lib_blob='#{addslashes(lib_code)}'")
      ret['ok'] = true
	...
	def submit_job(name, description, code, lib_filename, nb_splayds, churn_trace, options, session_id)
	 	#initializes the return variable
		ret = Hash.new
		#checks the validity of the session ID and stores the returning value in the variable user
		user = check_session_id(session_id)
		...
		if (user) then
			...
			$db.do("INSERT INTO jobs SET ref='#{ref}' #{to_sql(options)}, #{description_field} #{name_field} #{churn_field} code='#{addslashes(code)}', lib_name='#{lib_filename}' ,user_id=#{user_id}, created_at='#{time_now}'")
