...
base64_lib = loadfile("./lib/base64.lua")
base64_lib()
...
function send_submit_job(name, description, code_filename, lib_filename, nb_splayds, churn_trace_filename, options, job_args, cli_server_url, session_id)
	...
	local lib_code = nil
	local lib_hash = nil
	local do_submit_lib = false
	if lib_filename then
		do_submit_lib = true
			local lib_file = io.open(lib_filename)
			if lib_file then
				local lib_code_blob = lib_file:read("*a")
				lib_file:close()
				lib_code = enc(lib_code_blob)
				lib_hash = sha1(lib_code_blob)
				print(lib_hash)
				--print(lib_code)
			else 
				print("File \"" .. lib_filename .. "\" does not exist")
				os.exit()
			end
	end
	...
	local do_submit_lib_result = false
	local os_name = ""
	local arch = ""
	if do_submit_lib then 
		arch = io.popen("uname -m"):read()
		--arch = arch:read("*a")
		print(arch)
		os_name = io.popen("uname -s"):read()
		--os_name = os_name:read("*a")
		print(os_name)
		local body = json.encode({
			method = "ctrl_api.pre_submit_job",
			params = {name, lib_filename, os_name, arch, lib_hash}
		})
		...
		if response then
			...
			print(response)
			--print(json_response.result.ok)
			--print(json_response.result.message)
			--print(json_response.result.submit_sha1)
			do_submit_lib_result = json_response.result.ok
		end
	end 
	
	if do_submit_lib_result == false then
		print("will submit lib")
		local body = json.encode({
			method = "ctrl_api.submit_lib",
			params = {lib_filename, os_name, arch, lib_hash, lib_code}
		})
		local response = http.request(cli_server_url, body)
		if response then
			local json_response = json.decode(response)
	...
	local body = json.encode({
		...
		params = {name, description, code, lib_filename, nb_splayds, churn_trace, options, session_id}
		})
...
send_submit_job(name, description, code_filename, lib_filename,nb_splayds, churn_trace_filename, options, job_args, cli_server_url, session_id)
