...
function register(so)
	...
	prepare_lib_directory(job)
	job.status = "waiting"

	--print("job_fs ".. job.disk.directory)
	--print("job libs "..job.disk.lib_directory)
...
function prepare_lib_directory(job)
	if job.disk then
		job.disk.lib_directory = job.disk.directory.."/".."lib"
		splay.mkdir(job.disk.lib_directory)
		print("REGISTER DIR : "..job.disk.lib_directory) -- TODO: DELETE
		
		if job.lib_name and job.lib_name ~= "" then
			os.execute("ln -s "..libs_cache_dir.."/"..job.lib_name.. " " .. job.disk.lib_directory.."/"..job.lib_name)
		end
	else
...
function list_libs_in_cache(dir)
	...
	for lib_name in string.gmatch(libs_list, "%a+.so") do
...
function check_libs(so, libs_in_cache)
	print("check and encode "..#libs_in_cache.." libs")
	--assert(so:send())
	...
	local new_libs = json.decode(response)
	print(response)
	for i,v in pairs(new_libs) do
		print(v['name'])
		print(v['code'])
	end
...
function update_libs(dir, new_libs)
	print("Updating libs")
	print(new_libs)
	for i,v in pairs(new_libs) do
		print(v['name'])
		print(v['code'])
		-- suppress libs that don't exist anymore
		if (v['code'] == "") then
			print("rm "..dir.."/"..v['name'])
			io.popen("rm "..dir.."/"..v['name'])
		else
			local lib_file = io.open(dir.."/"..v['name'], "w+")
			lib_code = base64.dec(v['code']) 
...
function controller(so)
	...
	-- The splayd lists the libs it already has in his cache.
	-- Returns a table <lib, sha1>
	-- Then sends the list encoded to the controller and updates them if necessary
	-- TODO is it the right place to do so, I'd rather see this before the node is registred in the controller
	-- to ensure no message are sent before this.
	-- Apparently a new message is sufficient
	local libs_in_cache = list_libs_in_cache(libs_cache_dir)
	local new_libs = check_libs(so, libs_in_cache)
	update_libs(libs_cache_dir, new_libs)
...
--splayd.settings.job.disk.libs = jobs_libs_dir
