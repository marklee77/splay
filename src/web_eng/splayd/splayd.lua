...
function loadavg(so)
	...
	if splayd.status.os == "Linux" then
		...
		local bytes_transmitted = 0
		while true do
			local line = dev:read("*l")
			if line == nil then break end
			if string.find(line, "Inter-") == nil and string.find(line, "face") == nil and string.find(line, "lo:") == nil then
				f = string.gmatch(line, "%d%d+")
				bytes_received = bytes_received + f()
				f()
				bytes_transmitted = bytes_transmitted + f() 
			end
		end
		dev:close()
		--memory used (kb)
		local meminfo = io.open("/proc/meminfo","r")
		local mem_total = string.gmatch(meminfo:read(), "%d+")()
		local mem_free = string.gmatch(meminfo:read(), "%d+")()
		if mem_total and mem_free then
			mem_used = mem_total - mem_free
		else
			mem_used = -1 -- to signal error
		end
		meminfo:close()
		--diskload: sectors read, sectors written
		local diskstats = io.open("/proc/diskstats","r")
		local sectors_read = 0
		local sectors_written = 0
		while true do
			local line = diskstats:read("*l")
			if line == nil then break end
			if string.find(line, "ram") == nil and string.find(line, "loop") == nil then
				pos = 1
				for token in string.gmatch(line, "[^%s]+") do
					if pos == 6 then
						sectors_read = sectors_read + token
					end
					if pos == 10 then
						sectors_written = sectors_written + token
					end
					pos = pos + 1
				end
			end
		end
		sectors_written = sectors_written/2
		sectors_read = sectors_read/2
		diskstats:close()
		assert(so:send(cpu_load.." "..bytes_received.." "..bytes_transmitted.." "..mem_used.." "..sectors_read.." "..sectors_written))
...
if splayd.status.os == "Linux" then
	...
	sectors_read = sectors_read/2
	diskstats:close()
	splayd.status.loadavg = lf().." "..lf().." "..lf().." "..bytes_received.." "..bytes_transmitted.." "..mem_used.." "..sectors_read.." "..sectors_written
elseif splayd.status.os == "Darwin" then
	--now since Epoch
